...
def main():
    ...
    tab1, tab2, tab3, tab4, tab5 = st.tabs([
        "🗺️ City Heatmap", "🏙️ Metropolitan Areas", "📊 City Rankings", "🎯 Hotspot Analysis", "📈 Urban Insights"
    ])
...
def calculate_city_statistics(transaction_df):
    ...
    city_stats['risk_level'] = city_stats['avg_risk_score'].apply(
        lambda x: 'Critical' if x >= 70 else 'High' if x >= 60 else 'Medium' if x >= 40 else 'Low'
    )
...
def show_city_fraud_heatmap(predictions, fraud_probs, risk_scores, fraud_predictions, original_data):
    ...
    st.markdown("### 🗺️ US Cities Fraud Heatmap")
    ...
    with col1:
        heatmap_metric = st.selectbox(
            "Heatmap Metric",
            [
                "fraud_count", "fraud_rate", "avg_risk_score", "fraud_per_capita",
                "total_transactions", "high_risk_count", "transaction_density"
            ],
            format_func=lambda x: {
                "fraud_count": "Total Fraud Cases",
                "fraud_rate": "Fraud Rate (%)",
                "avg_risk_score": "Average Risk Score",
                "fraud_per_capita": "Fraud per 100K People",
                "total_transactions": "Total Transactions",
                "high_risk_count": "High Risk Cases",
                "transaction_density": "Transactions per 100K"
            }[x]
        )
    ...
    if len(filtered_stats) > 0:
        ...
        # Summary statistics
        st.markdown("#### 📊 Filtered Cities Summary")
        col1, col2, col3, col4, col5 = st.columns(5)
        
        with col1:
            st.metric("Cities Shown", len(filtered_stats))
        with col2:
        ...
        with col5:
            critical_cities = (filtered_stats['risk_level'] == 'Critical').sum()
...
def create_city_heatmap(city_stats, metric):
    ...
    for _, row in city_stats.iterrows():
        hover_text.append(
            ...
            f"Fraud per 100K: {row['fraud_per_capita']:.1f}"
        )
    
    # Size mapping based on population tier
    size_map = {1: 20, 2: 15, 3: 10}
    city_stats['marker_size'] = city_stats['tier'].map(size_map)
    
    # Color scale based on risk level
    color_map = {'Critical': '#8B0000', 'High': '#FF4444', 'Medium': '#FFA500', 'Low': '#4CAF50'}
    
    fig = go.Figure()
    
    # Add points for each risk level
    for risk_level in ['Low', 'Medium', 'High', 'Critical']:
        level_data = city_stats[city_stats['risk_level'] == risk_level]
        if len(level_data) > 0:
            fig.add_trace(go.Scattermapbox(
                ...
                mode='markers',
                marker=dict(
                    size=level_data['marker_size'],
                    color=color_map[risk_level],
                    opacity=0.8,
                    sizemin=8
                ),
                text=[hover_text[i] for i in level_data.index],
    ...
    fig.update_layout(
        title=f"US Cities - {metric.replace('_', ' ').title()}",
        mapbox=dict(
...
def show_metropolitan_analysis(predictions, fraud_probs, risk_scores, fraud_predictions, original_data):
    ...
    display_metro['fraud_per_capita'] = display_metro['fraud_per_capita'].round(1)
    ...
    simple_dataframe_fix(
        display_metro[['metro_area', 'total_transactions', 'fraud_count', 'fraud_rate', 
                      'avg_risk_score', 'fraud_per_capita', 'total_population']].rename(columns={
            'metro_area': 'Metropolitan Area',
            'total_transactions': 'Total Transactions',
            'fraud_count': 'Fraud Cases',
            'fraud_rate': 'Fraud Rate',
            'avg_risk_score': 'Avg Risk Score',
            'fraud_per_capita': 'Fraud per 100K',
...
def show_city_rankings(predictions, fraud_probs, risk_scores, fraud_predictions, original_data):
    ...
    ranking_tabs = st.tabs(["🎯 By Fraud Rate", "📊 By Volume", "👥 Per Capita", "💰 By Risk Score"])
    ...
    with ranking_tabs[2]:
        per_capita_ranking = ranked_cities.nlargest(20, 'fraud_per_capita')[
            ['city', 'state', 'fraud_per_capita', 'fraud_count', 'population', 'fraud_rate']
        ].copy()
        per_capita_ranking['fraud_rate'] = per_capita_ranking['fraud_rate'].apply(lambda x: f"{x:.2%}")
        per_capita_ranking['fraud_per_capita'] = per_capita_ranking['fraud_per_capita'].round(1)
        simple_dataframe_fix(per_capita_ranking, use_container_width=True)
...
def show_hotspot_analysis(predictions, fraud_probs, risk_scores, fraud_predictions, original_data):
    ...
    hotspots['hotspot_score'] = (
        ...
        (hotspots['fraud_per_capita'] / 10) * 0.2 +
...
def show_urban_insights(predictions, fraud_probs, risk_scores, fraud_predictions, original_data):
    ...
    st.markdown("#### 🏙️ City Size vs Fraud Patterns")
    ...
    pop_analysis = city_stats.groupby('pop_category').agg({
        'fraud_rate': 'mean',
        'avg_risk_score': 'mean',
        'fraud_per_capita': 'mean',
        'city': 'count'
    }).round(3)
    
    pop_analysis.columns = ['Avg_Fraud_Rate', 'Avg_Risk_Score', 'Avg_Fraud_Per_Capita', 'Cities_Count']
    ...
    regional_insights.columns = ['Cities', 'Avg_Fraud_Rate', 'Avg_Risk_Score', 
                               'Avg_Fraud_Per_Capita', 'Total_Transactions', 'Total_Fraud_Cases']
    ...
    display_regional['Avg_Fraud_Per_Capita'] = display_regional['Avg_Fraud_Per_Capita'].round(1)
    ...
    simple_dataframe_fix(
        display_regional.rename(columns={
            ...
            'Avg_Risk_Score': 'Avg Risk Score',
            'Avg_Fraud_Per_Capita': 'Fraud per 100K',
            'Total_Transactions': 'Total Transactions',
